...
export function ApiSlider(): JSX.Element {
  ...
  const apiWidgets = [
      ...
      value: (
        <div className="flex justify-center items-center gap-[20px]">
          <div className="text-center">
            <div className="text-[1.2rem] mb-[5px]">{weatherIcon}</div>
            <div className="text-[1rem] opacity-80">{t.bento.now}</div>
            <div className="font-bold">{weatherToday}</div>
          </div>
          <div className="w-px h-[40px] bg-[var(--color-lilas-doux)] opacity-30"></div>
          <div className="text-center">
            <div className="text-[1.2rem] mb-[5px]">📅</div>
    ...
    },
    {
      id: 'spotify',
      icon: (
        <div className="w-[75px] h-[75px] rounded-[15px] shadow-[4px_4px_10px_var(--shadow-dark),-4px_-4px_10px_var(--shadow-light)] overflow-hidden flex items-center justify-center bg-[var(--card-bg)]">
      ...
      value: (
        <div className="flex flex-col items-center">
          <div className="flex flex-col gap-[2px] w-full overflow-hidden text-center">
            <div className="font-bold text-[1.2rem] text-[var(--color-lilas-vif)]">
          ...
          {spotifyData.message?.includes('Aucun compte') ? (
            <button
              className="w-full text-[0.8rem] rounded-[20px] bg-[var(--color-lilas-doux)] border-0 text-white"
              onClick={handleConnectSpotify}
            >
              {t.bento.connect}
            </button>
          ) : (
            <div className="mb-[5px] flex gap-[15px] justify-center items-center">
              <button
                onClick={() => handleSpotifyAction('prev')}
                className="bg-none border-none text-[1.1rem] p-[5px] text-[var(--color-profond)] hover:scale-[1.2] transition-transform duration-100"
              >
                ⏮️
              </button>
              <button
                onClick={() => handleSpotifyAction(spotifyData.isPlaying ? 'pause' : 'play')}
                className="bg-none border-none text-[1.1rem] p-[5px] text-[var(--color-profond)] hover:scale-[1.2] transition-transform duration-100 text-[1.4rem]"
              >
  ...
  return (
    <Card className="relative flex flex-col justify-center items-center text-center w-full flex-1 min-h-0 max-[1350px]:flex-none max-[1350px]:min-h-[25vh]">
      <button
        className="absolute top-1/2 -translate-y-1/2 w-[30px] h-[30px] rounded-full border-none bg-[var(--card-bg)] text-[var(--color-lilas-vif)] shadow-[2px_2px_5px_var(--shadow-dark),-2px_-2px_5px_var(--shadow-light)] z-10 flex items-center justify-center text-[1.2rem] transition-all duration-200 pb-[2px] hover:bg-[var(--color-rose-poudre)] hover:text-white hover:shadow-[inset_2px_2px_5px_rgba(0,0,0,0.1)] left-[10px]"
        onClick={() => setApiIndex((prev) => (prev - 1 + apiWidgets.length) % apiWidgets.length)}
      >
        ‹
      </button>
      <div className="flex-1 flex flex-col justify-center items-center w-full gap-[8px]">
        <div>{apiWidgets[apiIndex].icon}</div>
        <div>
          <div>{apiWidgets[apiIndex].value}</div>
          <small className="text-[var(--color-lilas-doux)] text-[0.8rem]">
            {apiWidgets[apiIndex].label}
          </small>
        </div>
      </div>
      <button
        className="absolute top-1/2 -translate-y-1/2 w-[30px] h-[30px] rounded-full border-none bg-[var(--card-bg)] text-[var(--color-lilas-vif)] shadow-[2px_2px_5px_var(--shadow-dark),-2px_-2px_5px_var(--shadow-light)] z-10 flex items-center justify-center text-[1.2rem] transition-all duration-200 pb-[2px] hover:bg-[var(--color-rose-poudre)] hover:text-white hover:shadow-[inset_2px_2px_5px_rgba(0,0,0,0.1)] right-[10px]"
      ...
      <div className="absolute bottom-[12px] left-1/2 -translate-x-1/2 flex gap-[6px]">
        {apiWidgets.map((_, i) => (
          <div
            key={i}
            className={`w-[6px] h-[6px] rounded-full bg-[var(--shadow-dark)] transition-all duration-300 ${
              i === apiIndex ? 'bg-[var(--color-lilas-vif)]' : ''
            }`}
          />
        ))}
      </div>
